...
export default function SignInPage() {
  ...
  const handleGoogle = () => {
    if (!googleEnabled) {
      toast.error('Google login needs OAuth credentials first.')
      return
    }
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-12">
        <motion.div className="w-full max-w-md"
          ...
          <button
            ...
            type="button"
            className="w-full flex items-center justify-center gap-3 border border-ink/20 py-3 text-sm hover:border-ink transition-colors mb-6 disabled:opacity-60 disabled:hover:border-ink/20"
          >
            ...
            {googleEnabled ? 'Continue with Google' : 'Google login needs setup'}
          </button>

          <div className="flex items-center gap-4 mb-6">
            <div className="flex-1 h-px bg-ink/10" />
            <span className="text-xs text-smoke font-mono">OR</span>
            <div className="flex-1 h-px bg-ink/10" />
          </div>
