  ...
  const [orders, total] = await Promise.all([
    prisma.order.findMany({
      ...
      include: {
        ...
        fulfillment: { select: { status: true, providerOrderId: true, trackingNumber: true } },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-ink/10 text-left">
                {['Order', 'Customer', 'Date', 'Items', 'Total', 'Status', 'Fulfillment', 'Actions'].map((h) => (
                ...
                <tr key={order.id} className="hover:bg-cream-dark/30 transition-colors">
                  ...
                  <td className="py-3 pr-4">
                    <div className="text-xs">
                      <p className="font-mono text-smoke">{order.fulfillment?.status || '—'}</p>
                      {order.fulfillment?.trackingNumber && (
                        <p className="text-ink font-mono">{order.fulfillment.trackingNumber}</p>
                      )}
                    </div>
                  </td>
                  <td className="py-3 pr-4">
                    <AdminOrderActions orderId={order.id} currentStatus={order.status}
                      hasFulfillment={!!order.fulfillment?.providerOrderId} />
                  </td>
                </tr>
              ))}
            </tbody>
